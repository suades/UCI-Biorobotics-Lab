...
readings from an LSM6DS33 to calculate a tilt-compensated magnetic heading. It
depends on Pololu's LIS3MDL and LSM6 Arduino libraries, which can be installed
...
void loop()
{
  ...
  computeHeading() that takes a vector argument; for example, use

    computeHeading((LIS3MDL::vector<int>){0, 0, -1});

  to use the -Z axis as a reference.
  */
  float heading_x = computeHeading();
  
  Serial.println(heading_x);
  delay(100);
}
